  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [
        ...
        supabase.from('companies').select('id, company_name, contact_name, email, created_at, contact_id').order('created_at', { ascending: false }).limit(30),
  ...
  return (
    <div style={{ display: 'grid', gap: 20, maxWidth: 1400 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
        {[
          { label: 'Companies', value: loading ? '…' : kpis.companies, icon: '🏢', soft: C.blueSoft, border: C.blueBorder, text: C.blue },
          { label: 'Active Contracts', value: loading ? '…' : kpis.activeContracts, icon: '📄', soft: C.primarySoft, border: '#bbf7d0', text: '#166534' },
          { label: 'Open Deals', value: loading ? '…' : kpis.openDeals, icon: '💼', soft: C.warnSoft, border: C.warnBorder, text: C.warn },
          { label: 'Expiring (90d)', value: loading ? '…' : kpis.expiringContracts, icon: '⏰', soft: kpis.expiringContracts > 0 ? C.dangerSoft : C.primarySoft, border: kpis.expiringContracts > 0 ? C.dangerBorder : '#bbf7d0', text: kpis.expiringContracts > 0 ? C.danger : '#166534' },
        ].map(({ label, value, icon, soft, border, text }) => (
          <div key={label} style={{ background: soft, border: `1px solid ${border}`, borderRadius: 12, padding: '16px 18px', boxSizing: 'border-box' }}>
