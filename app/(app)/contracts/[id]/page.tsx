...
function statusTone(status?: string | null): 'neutral' | 'green' | 'amber' | 'red' | 'gray' | 'blue' {
...
function ChangesPretty({ changes }: { changes: any }) {
  if (!changes || typeof changes !== 'object') {
    return <div style={{ fontSize: 12, color: ui.color.text3 }}>No structured changes.</div>;
  }

  const expiry = changes.expiry_date ? String(changes.expiry_date) : '';
  const paymentTerm = changes.payment_term != null ? String(changes.payment_term) : '';
  const paymentType = changes.payment_type ? String(changes.payment_type) : '';

  const platformsAdd: string[] = changes?.platforms?.add ? (Array.isArray(changes.platforms.add) ? changes.platforms.add : []) : [];
  const platformsRemove: string[] = changes?.platforms?.remove ? (Array.isArray(changes.platforms.remove) ? changes.platforms.remove : []) : [];

  const rows: { label: string; value: React.ReactNode }[] = [];

  if (expiry) rows.push({ label: 'Expiry date', value: formatDate(expiry) });
  if (paymentTerm) rows.push({ label: 'Payment term', value: `${paymentTerm} days` });
    ...
    <div style={{ display: 'grid', gap: 8, marginTop: 6 }}>
      {rows.map((r, idx) => (
        <div key={idx} style={{ display: 'grid', gap: 4 }}>
          <div style={{ fontSize: 12, color: ui.color.text3, fontWeight: 800 }}>{r.label}</div>
...
function ContractDetailPage() {
  ...
  const [selectedRebateContractId, setSelectedRebateContractId] = useState<string>('');
  ...
  function makeSnapshot(v: {
    ...
    return stableStringify({
      ...
      selectedPlatforms: Array.from(new Set(v.selectedPlatforms)).map((x) => String(x)).sort(),
  ...
  function hydrateFromRow(r: ContractRow, cps: ContractPlatformRow[]) {
    ...
    setPaymentType(((r.payment_type as PaymentType) || 'Post-payment') as PaymentType);
    ...
    const snap = makeSnapshot({
      ...
      paymentType: (((r.payment_type as PaymentType) || 'Post-payment') as PaymentType),
  ...
  async function loadCurrentState() {
    ...
    try {
      const { data, error } = await supabase.rpc('get_contract_current_state', { p_contract_id: contractId });
      if (error) {
        console.error(error);
  ...
  async function reloadPlatforms() {
    const { data, error } = await supabase.from('contract_platforms').select('*').eq('contract_id', contractId);
    if (error) {
      console.error(error);
      return;
    }
  ...
  async function loadRebateLink() {
    ...
    try {
      ...
      if (direct.data) {
        ...
        setRebateLink(link);
        setSelectedRebateContractId(link.linked_contract_id ? String(link.linked_contract_id) : '');
        return;
      }
      ...
      setRebateLink(link);
      setSelectedRebateContractId(link?.contract_id ? String(link.contract_id) : '');
    } finally {
  ...
  async function loadRebateContractsForCompany(company_id: string | null) {
    ...
    try {
      ...
      const filtered = all
        ...
        .filter((c) => (safeTrim(c.contract_type || '').toUpperCase().startsWith('REB')));
  ...
  async function persistRebateLink(selectedIdStr: string, company_id: string | null) {
    ...
    if (!selectedId) {
      await supabase
        .from('contract_links')
        .delete()
        .or(`and(contract_id.eq.${contractId},link_type.eq.REBATE),and(linked_contract_id.eq.${contractId},link_type.eq.REBATE)`);
      return;
    }
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [companiesRes, contractRes, cpRes] = await Promise.all([
        ...
        supabase.from('contract_platforms').select('*').eq('contract_id', contractId),
      ]);
  ...
  const dateError = useMemo(() => {
    ...
    const x = expiryDate;
    if (i && e && cmpYMD(e, i) < 0) return 'Effective date cannot be before Issue date.';
    if (e && x && cmpYMD(x, e) < 0) return 'Expiry date cannot be before Effective date.';
    return null;
  }, [issueDate, effectiveDate, expiryDate]);

  const companyOptions = useMemo(
    () => [{ value: '', label: 'Select company…' }, ...companies.map((c) => ({ value: c.id, label: c.company_name }))],
    [companies]
  );
  ...
  const wonDealOptions = useMemo(() => {
    const base = [{ value: '', label: dealsLoading ? 'Loading deals…' : 'Not selected' }];
  ...
  const rebateOptions = useMemo(() => {
    const base = [{ value: '', label: rebateContractsLoading ? 'Loading rebate contracts…' : 'Not linked' }];
  ...
  const activePlatformCodes = useMemo(() => {
    return Array.from(new Set(platformRows.filter((x) => x.active).map((x) => x.platform_code))).sort();
  }, [platformRows]);
  ...
  async function handleSaveContract() {
    ...
    if (pt && (Number.isNaN(ptNum) || (ptNum as number) < 0)) return alert('Payment term must be a number of days (>= 0).');

    const url = safeTrim(signedUrl);
    if (!isValidUrl(url)) return alert('Signed contract URL must be a valid URL (include https://).');

    const pId = safeTrim(primaryWonDealId);
    if (pId) {
      const ok = wonDeals.some((d) => d.id === pId);
      if (!ok) return alert('Selected Primary Won deal is not valid for the current company. Please re-select.');
    }
    ...
    if (rebStr) {
      ...
      if (!ok) return alert('Selected rebate contract is not valid for this company. Please re-select.');
    }

    setSaving(true);
    try {
      const selectedCompany = companies.find((c) => c.id === safeTrim(companyId));
      const resolvedCompanyName = selectedCompany?.company_name ?? safeTrim(companyNameCache) ?? null;
      ...
      await persistRebateLink(rebStr, safeTrim(companyId) ? safeTrim(companyId) : null);
      await loadRebateLink(); // refresh after save

      // platforms reconcile
      const existing = platformRows || [];
      const existingActive = new Set(existing.filter((x) => x.active).map((x) => x.platform_code));
      ...
      if (toActivate.length) {
        ...
        const { error } = await supabase.from('contract_platforms').insert(rows);
        if (error) console.error(error);
      }
      ...
      await Promise.all([loadWonDealsForCompany(nextRow.company_id), loadCurrentState(), loadRebateContractsForCompany(nextRow.company_id)]);
      ...
      const snap = makeSnapshot({
        ...
        paymentType: ((nextRow.payment_type as PaymentType) || 'Post-payment') as PaymentType,
        paymentTerm: nextRow.payment_term != null ? String(nextRow.payment_term) : '',
  ...
  async function handleCreateAddendum() {
    ...
    if (!eff) return alert('Addendum: effective_from is required (YYYY-MM-DD).');

    if (addSignedUrl && !isValidUrl(addSignedUrl)) {
      return alert('Addendum signed URL must be a valid URL (include https://).');
    }
    ...
    if (ptTrim) {
      ...
      if (Number.isNaN(n) || n < 0) return alert('Addendum payment_term must be a number >= 0.');
  ...
  async function handleSaveAddendumEdit(id: number) {
    ...
    if (editSignedUrl.trim()) {
      try { new URL(editSignedUrl.trim()); } catch { return alert('Signed URL must be a valid URL (include https://).'); }
    }
  ...
  if (loading) {
    return <div style={{ padding: 16, color: ui.color.text3, fontSize: 13 }}>Loading contract…</div>;
  }

  if (!row) {
    return (
      <div style={{ padding: 16, display: 'grid', gap: 10 }}>
        <div style={{ color: ui.color.text, fontWeight: 800, fontSize: 14 }}>Contract not found.</div>
        <Link href="/contracts" style={{ textDecoration: 'none', color: ui.color.primary, fontWeight: 700 }}>
          ← Back to contracts
        </Link>
      </div>
    );
  ...
  return (
    <div style={{ display: 'grid', gap: ui.space.md, padding: ui.space.lg, boxSizing: 'border-box' }}>
      <PageHeader
        title={headerTitle}
        ...
        right={
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
            ...
            {tab !== 'edit' ? (
              <Button variant="primary" size="sm" onClick={() => setTab('edit')}>
                Edit
              </Button>
            ) : (
              <>
                <Button variant="secondary" size="sm" onClick={handleCancelEdits} disabled={saving}>
                  Cancel
                </Button>
                <Button variant="primary" size="sm" disabled={saving || !!dateError || !isDirty} onClick={handleSaveContract}>
                  {saving ? 'Saving…' : 'Save'}
                </Button>
              </>
            )}
      ...
      <Card style={{ padding: 10 }}>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          <button type="button" style={tabBtn(tab === 'addendums')} onClick={() => setTab('addendums')}>
          ...
          <div style={{ display: 'grid', gap: 10 }}>
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              <div style={{ fontSize: 12, color: ui.color.text3 }}>
                Created: {formatDate(row.created_at)} • Party: {String(row.party_type || '—')} • Payment: {String(row.payment_type || '—')}
              </div>
              <Button variant="secondary" size="sm" onClick={() => loadCurrentState()} disabled={currentStateLoading}>
                {currentStateLoading ? 'Refreshing…' : 'Refresh current'}
              </Button>
            </div>

            <div style={grid}>
              <Info label="Current payment term" value={currentState?.payment_term != null ? `${currentState.payment_term} days` : '—'} />
              <Info label="Current payment type" value={currentState?.payment_type || '—'} />
              <Info label="Current expiry date" value={currentState?.expiry_date ? formatDate(currentState.expiry_date) : '—'} />
              <Info label="Current platforms" value={currentState?.platforms?.length ? currentState.platforms.join(', ') : '—'} />
            </div>

            <div style={grid}>
              <Info label="Company" value={companyLabel} />
              <Info label="Contract number" value={row.contract_number || '—'} />
              ...
              <Info label="Effective date" value={formatDate(row.effective_date)} />
              <Info label="Expiry date (base)" value={formatDate(row.expiry_date)} />
              <Info label="Automatic renewal" value={row.automatic_renewal ? 'Yes' : 'No'} />
              <Info label="Payment term (base)" value={row.payment_term != null ? `${row.payment_term} days` : '—'} />

              <Info
                label="Primary won deal"
                value={
                  row.primary_won_deal_id ? (
                    <Link href={`/deals/${row.primary_won_deal_id}`} style={{ textDecoration: 'none', color: ui.color.primary, fontWeight: 700 }}>
              ...
              <Info
                ...
                value={
                  linkedRebateId ? (
                    <Link href={`/contracts/${linkedRebateId}`} style={{ textDecoration: 'none', color: ui.color.primary, fontWeight: 700 }}>
                ...
                value={
                  row.signed_contract_url ? (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      style={{ color: ui.color.primary, textDecoration: 'none', fontWeight: 700 }}
                    >
                      Open →
                    </a>
                  ) : (
              ...
              <Info label="Platforms (base active)" value={activePlatformCodes.length ? activePlatformCodes.join(', ') : '—'} />

              {rebateErr ? (
                <div style={{ gridColumn: '1 / -1', fontSize: 12, color: '#b91c1c', fontWeight: 700 }}>
      ...
      {tab === 'edit' && (
        <Card>
          <div style={{ display: 'grid', gap: ui.space.sm }}>
            <div style={grid}>
              <Select label="Company" value={companyId} onChange={(v) => setCompanyId(v)} options={companyOptions} disabled={saving} />
              <Input
                ...
                disabled={saving}
              />
              <Input label="Contract number" value={contractNumber} onChange={setContractNumber} disabled={saving} />
              <Select label="Status" value={status} onChange={(v) => setStatus(v)} options={statusOptions} disabled={saving} />
            </div>

            <div style={grid}>
              <Select label="Party type" value={partyType} onChange={(v) => setPartyType(v as PartyType)} options={partyOptions} disabled={saving} />
              <Input label="Contract type" value={contractType} onChange={setContractType} placeholder="GMP / TT / TDS" disabled={saving} />
              <Input label="Contract owner" value={contractOwner} onChange={setContractOwner} placeholder="Ioana / Zoli / etc." disabled={saving} />
              <Select label="Payment type" value={paymentType} onChange={(v) => setPaymentType(v as PaymentType)} options={paymentTypeOptions} disabled={saving} />
            </div>

            <div style={grid}>
              <InputDate label="Issue date" value={issueDate} onChange={setIssueDate} disabled={saving} />
              <InputDate label="Effective date" value={effectiveDate} onChange={setEffectiveDate} disabled={saving} min={issueDate || undefined} />
              <InputDate label="Expiry date (base)" value={expiryDate} onChange={setExpiryDate} disabled={saving} min={(effectiveDate || issueDate) || undefined} />
              <div style={{ minWidth: 0 }}>
                <label style={{ fontSize: 12, color: ui.color.text3, marginBottom: 6, display: 'block' }}>Automatic renewal</label>
                <div style={{ display: 'flex', gap: 0, height: 40, borderRadius: 8, overflow: 'hidden', border: `1px solid ${ui.color.border}` }}>
                  {([true, false] as const).map((val) => (
                    <button
                      ...
                      style={{
                        flex: 1,
                        border: 'none',
                        borderRight: val ? `1px solid ${ui.color.border}` : 'none',
                        background: automaticRenewal === val ? (val ? '#dcfce7' : '#fee2e2') : '#fff',
                        color: automaticRenewal === val ? (val ? '#166534' : '#991b1b') : ui.color.text3,
            ...
            <div style={grid}>
              <Input label="Payment term (days)" value={paymentTerm} onChange={setPaymentTerm} placeholder="e.g. 30" disabled={saving} />
              ...
              <Input label="Signed contract URL" value={signedUrl} onChange={setSignedUrl} placeholder="https://drive.google.com/..." disabled={saving} />
              <Input label="Notes" value={notes} onChange={setNotes} placeholder="Optional notes…" disabled={saving} />

              <div style={{ fontSize: 12, color: ui.color.text3, paddingBottom: 6 }}>
                {dateError ? <span style={{ color: '#b91c1c', fontWeight: 700 }}>{dateError}</span> : isDirty ? 'You have changes not saved.' : 'All changes saved.'}
              </div>
            </div>

            {/* ✅ Linked rebate contract (EDIT) - NO extra buttons (Refresh/Save link/Remove link removed as requested) */}
            <div style={{ marginTop: 6 }}>
              <div style={{ fontSize: 12, color: ui.color.text3, marginBottom: 6, fontWeight: 800 }}>Linked rebate contract</div>
              <div style={{ ...grid }}>
                <div style={{ minWidth: 0 }}>
                  <Select
                  ...
                  />
                  <div style={{ marginTop: 6, fontSize: 12, color: ui.color.text3 }}>
                    Pick an existing <b>REB</b> contract for the same company. It will be linked when you click <b>Save</b>.
                  </div>
                </div>

                <div style={{ minWidth: 0 }}>
                  <label style={{ fontSize: 12, color: ui.color.text3, marginBottom: 6, display: 'block' }}>Current linked</label>
                  <div style={{ height: 40, display: 'flex', alignItems: 'center' }}>
                    {linkedRebateId ? (
                      ...
                      <span style={{ fontSize: 13, color: ui.color.text2 }}>—</span>
                    )}
                  </div>
                </div>
              </div>

              {rebateErr ? <div style={{ marginTop: 8, fontSize: 12, color: '#b91c1c', fontWeight: 700 }}>{rebateErr}</div> : null}
            </div>

            {/* Platforms */}
            <div>
              <div style={{ fontSize: 12, color: ui.color.text3, marginBottom: 6, fontWeight: 700 }}>Platforms in contract (base)</div>
                ...
                {PLATFORMS.map((p) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        if (saving) return;
                        setSelectedPlatforms((prev) => (checked ? prev.filter((x) => x !== p.code) : [...prev, p.code]));
                      }}
                      style={{
                        padding: '8px 10px',
                        borderRadius: 999,
                        border: `1px solid ${checked ? ui.color.primary : ui.color.border}`,
                        background: checked ? ui.color.primarySoft : ui.color.surface,
              ...
              <div style={{ marginTop: 10, fontSize: 12, color: ui.color.text3 }}>
                Saved platforms (base active): <span style={{ fontWeight: 700 }}>{activePlatformCodes.length ? activePlatformCodes.join(', ') : '—'}</span>
              </div>
            </div>
      ...
      {tab === 'addendums' && (
        <Card style={{ padding: 14 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
            <div style={{ fontSize: 13, fontWeight: 800, color: ui.color.text }}>Addendums</div>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <Button variant="secondary" size="sm" onClick={() => loadAddendums()}>
                Refresh
              </Button>
              <Button variant="secondary" size="sm" onClick={() => loadCurrentState()}>
                Refresh current
              </Button>
            </div>
          </div>
          ...
          {/* Create addendum */}
          <div style={{ marginTop: 12, borderTop: `1px solid ${ui.color.border}`, paddingTop: 12 }}>
            <div style={{ fontSize: 13, fontWeight: 800, color: ui.color.text }}>Create addendum</div>

            <div style={{ marginTop: 10, ...grid }}>
              <Input label="Addendum number (optional)" value={addAddendumNumber} onChange={setAddAddendumNumber} placeholder="A1" />
              <InputDate label="Effective from (required)" value={addEffectiveFrom} onChange={setAddEffectiveFrom} />
              <InputDate label="Signed date (optional)" value={addSignedDate} onChange={setAddSignedDate} />
              <Input label="Signed URL (optional)" value={addSignedUrl} onChange={setAddSignedUrl} placeholder="https://..." />
            </div>

            <div style={{ marginTop: 10, ...grid }}>
              <InputDate label="New expiry date (optional)" value={addNewExpiry} onChange={setAddNewExpiry} />
              <Input label="New payment term days (optional)" value={addNewPaymentTerm} onChange={setAddNewPaymentTerm} placeholder="e.g. 45" />
              <Select
              ...
              />
              <Input label="Notes (optional)" value={addNotes} onChange={setAddNotes} placeholder="What changed and why…" />
            </div>

            {/* Platforms add/remove */}
            <div style={{ marginTop: 10 }}>
              <div style={{ fontSize: 12, color: ui.color.text3, fontWeight: 800, marginBottom: 6 }}>Platforms delta</div>

              <div style={{ display: 'grid', gap: 10 }}>
                <div>
                  <div style={{ fontSize: 12, color: ui.color.text3, fontWeight: 700, marginBottom: 6 }}>Add platforms</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                    {PLATFORMS.map((p) => {
                      ...
                      return (
                        <button
                          key={`add-${p.code}`}
                          type="button"
                          onClick={() => setAddPlatformsAdd((prev) => (checked ? prev.filter((x) => x !== p.code) : [...prev, p.code]))}
                          style={{
                            padding: '8px 10px',
                            borderRadius: 999,
                            border: `1px solid ${checked ? ui.color.primary : ui.color.border}`,
                ...
                <div>
                  <div style={{ fontSize: 12, color: ui.color.text3, fontWeight: 700, marginBottom: 6 }}>Remove platforms</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                    {PLATFORMS.map((p) => {
                      ...
                      return (
                        <button
                          key={`rem-${p.code}`}
                          type="button"
                          onClick={() => setAddPlatformsRemove((prev) => (checked ? prev.filter((x) => x !== p.code) : [...prev, p.code]))}
                          style={{
                            padding: '8px 10px',
                            borderRadius: 999,
                            border: `1px solid ${checked ? '#b91c1c' : ui.color.border}`,
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                  <Button variant="primary" size="sm" onClick={handleCreateAddendum} disabled={creatingAddendum}>
          ...
          {/* History */}
          <div style={{ marginTop: 14, borderTop: `1px solid ${ui.color.border}`, paddingTop: 12 }}>
            <div style={{ fontSize: 13, fontWeight: 800, color: ui.color.text }}>History</div>

            {addendums.length === 0 ? (
              <div style={{ marginTop: 10, color: ui.color.text3, fontSize: 13 }}>No addendums yet.</div>
            ) : (
                ...
                {addendums.map((a) => (
                  ...
                  >
                    {editingAddendumId === a.id ? (
                      /* ── EDIT MODE ── */
                      <div style={{ display: 'grid', gap: 10 }}>
                        <div style={{ fontSize: 13, fontWeight: 700, color: ui.color.text, marginBottom: 2 }}>
                          Editing addendum
                        </div>
                        ...
                        <div style={{ display: 'flex', gap: 8 }}>
                          ...
                          <Button variant="secondary" size="sm" onClick={cancelEditAddendum}>
                            Cancel
                          </Button>
                        </div>
                      </div>
                      ...
                      <>
                        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
                          <div style={{ fontSize: 13, fontWeight: 800, color: ui.color.text }}>
                            {a.addendum_number ? `Addendum ${a.addendum_number}` : `Addendum #${a.id}`}
                          </div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                            <div style={{ fontSize: 12, color: ui.color.text3 }}>
                              Effective from: <span style={{ fontWeight: 700 }}>{formatDate(a.effective_from)}</span>
                              {a.signed_date ? ` • Signed: ${formatDate(a.signed_date)}` : ''}
                            </div>
                            <Button variant="secondary" size="sm" onClick={() => startEditAddendum(a)}>
                              Edit
                            </Button>
                          </div>
                        </div>

                        <div style={{ borderTop: `1px solid ${ui.color.border}`, paddingTop: 8 }}>
                          <div style={{ fontSize: 12, color: ui.color.text3, fontWeight: 800 }}>Changes</div>
                        ...
                        {a.signed_url ? (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            style={{ fontSize: 12, color: ui.color.primary, fontWeight: 700, textDecoration: 'none' }}
                          >
                            Open signed addendum →
                          </a>
                        ) : null}

                        {a.notes ? <div style={{ fontSize: 12, color: ui.color.text2 }}>{a.notes}</div> : null}
                      </>
                    )}
                  </div>
                ))}
...
function Info({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    ...
    >
      <div style={{ fontSize: 12, color: ui.color.text3, fontWeight: 600 }}>{label}</div>
      <div style={{ marginTop: 6, fontSize: 13, fontWeight: 700, color: ui.color.text, wordBreak: 'break-word' }}>{value}</div>
    </div>
  );
}